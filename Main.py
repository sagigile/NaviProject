...
C = 299792458.0
SMOOTH_WINDOW = 7  # חלון לניקוי קפיצות
...
if __name__ == "__main__":
    ...
    # שלב 1: חישוב כל הנקודות הגולמיות
    ...
    # שלב 2: ניקוי רעשים עם Median Filter (הכי חזק נגד קפיצות)
    ...
    # שלב 3: המרה ל-LLA ויצירת תוצרים
    results = []
    kml = simplekml.Kml()
    coords_for_kml = []
    ...
    for i in range(len(smoothed_ecef)):
        ...
        # חישוב מהירות (מטר לשנייה)
        vel = 0.0
        if i > 0:
            p1 = smoothed_ecef.iloc[i - 1]
            p2 = smoothed_ecef.iloc[i]
            vel = np.linalg.norm(p2 - p1)
            if vel > 40: vel = results[-1]['Velocity (m/s)']  # סינון קפיצות מהירות
